...
As discussed in {ref}`Project Code`, there are 5 `.py` files directly under the `mesh4d/` folder. These are 4 **core modules** ({mod}`mesh4d.obj4d`, {mod}`mesh4d.obj3d`, {mod}`mesh4d.kps`, {mod}`mesh4d.field`) and 1 **auxiliary modules** ({mod}`mesh4d.utils`) of this project.
...
Actually, the whole {mod}`mesh4d` project is motivated and centred around this bottleneck problem. We aims at revealing the so-called inner-relationship information between different frames. **An analysable 4D scene must consists such information**. At the most meticulous level, the inner-relationship information can be represented as a *displacement field*. That is, specifically, in which direction and to what distance a point in a 3D frame is moving.

## Construction of the analysable 4D scene

The development of the core modules and 1 auxiliary modules ({mod}`mesh4d.utils`) is centred around constructing an {ref}`analysable 4D scene <What makes an analysable 4D scene?>`. It follows such a pattern:

- The 4D object defined in {mod}`mesh4d.obj4d` contains of a series of 3D objects.
- The 3D object defined in {mod}`mesh4d.obj3d` contains the loaded mesh, point cloud, key point coordinates (provided by Vicon), and the estimated displacement field.
- Tools for handling key point coordinates and estimating displacement field are provided in {mod}`mesh4d.kps` and {mod}`mesh4d.field`, respectively.
...
With inheritance, the development of classes can lay out in an incremental fashion. If you jump into {mod}`mesh4d.obj3d` and {mod}`mesh4d.obj4d`, you will find that they all contain 3 classes, 1 without suffix and 2 with suffixe `_Kps` or `_Deform`:

- The classes without suffix ({class}`~mesh4d.obj3d.Obj3d` and {class}`~mesh4d.obj4d.Obj4d`) are the basic classes for 3D and 4D objects. Only basic features like loading from 3dMD scanning data and sampling the point cloud are realised.
- The classes with suffix `_Kps` ({class}`~mesh4d.obj3d.Obj3d_Kps` and {class}`~mesh4d.obj4d.Obj4d_Kps`) are derived from {class}`~mesh4d.obj3d.Obj3d` and {class}`~mesh4d.obj4d.Obj4d`, respectively. The major development is attaching key points ({class}`~mesh4d.kps.Kps`) to it.
- The classes with suffix `_Deform` ({class}`~mesh4d.obj3d.Obj3d_Deform` and {class}`~mesh4d.obj4d.Obj4d_Deform`) are derived from {class}`~mesh4d.obj3d.Obj3d_Kps` and {class}`~mesh4d.obj4d.Obj4d_Kps`, respectively. The major development is attaching the displacement field ({class}`~mesh4d.field.Trans_Rigid` and {class}`~mesh4d.field.Trans_Nonrigid`) to it.

```{tip}
At the page of each module, such as {mod}`mesh4d.obj4d`, an inheritance relationship graph is shown under the table of classes.
```
...
```{danger}
The extended classes should be placed in {mod}`mesh4d.analysis` sub-package. Since the core modules are providing a skeleton for all downstream classes, it shall not be modified unless there are very strong reasons to do so, otherwise unpredictable issues may emerge.
```
